      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 16px 100px', position: 'relative', zIndex: 1 }}>
        ...
        {tab === 'profile' && (() => {
          ...
          const getDocStatus = (docKey) => {
            const stored = localStorage.getItem(`indoo_driver_docs_${docKey}`)
          ...
          const handleDocUpload = (docKey, file) => {
            ...
            reader.onload = (ev) => {
              localStorage.setItem(`indoo_driver_docs_${docKey}`, JSON.stringify({ preview: ev.target.result, approved: false, uploadedAt: new Date().toISOString() }))
              setTab('_refresh')
          ...
          const savedNpwp = localStorage.getItem('indoo_driver_npwp') || ''
          const savedBank = JSON.parse(localStorage.getItem('indoo_driver_bank') || '{}')
          const savedEmergency = JSON.parse(localStorage.getItem('indoo_driver_emergency') || '{}')
            ...
            <>
              ...
              <div style={{ ...GLASS, padding: 18, marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div>
                    <label style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: 6 }}>NPWP (Nomor Pokok Wajib Pajak) — Tax ID Number</label>
                    <input type="text" placeholder="Enter NPWP number" defaultValue={savedNpwp} onBlur={e => localStorage.setItem('indoo_driver_npwp', e.target.value)} style={{ width: '100%', padding: '12px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: 14, fontWeight: 600, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: 6 }}>Bank Name</label>
                    <select defaultValue={savedBank.bankName || ''} onChange={e => { const b = JSON.parse(localStorage.getItem('indoo_driver_bank') || '{}'); b.bankName = e.target.value; localStorage.setItem('indoo_driver_bank', JSON.stringify(b)) }} style={{ width: '100%', padding: '12px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: 14, fontWeight: 600, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box', appearance: 'none' }}>
                      <option value="" disabled style={{ color: '#888' }}>Select bank...</option>
                      {['BCA', 'Mandiri', 'BNI', 'BRI', 'BSI', 'CIMB', 'Permata', 'Danamon'].map(b => (
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="Enter account number" defaultValue={savedBank.accountNumber || ''} onBlur={e => { const b = JSON.parse(localStorage.getItem('indoo_driver_bank') || '{}'); b.accountNumber = e.target.value; localStorage.setItem('indoo_driver_bank', JSON.stringify(b)) }} style={{ width: '100%', padding: '12px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: 14, fontWeight: 600, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: 6 }}>Account Holder Name</label>
                    <input type="text" placeholder="Enter account holder name" defaultValue={savedBank.holderName || ''} onBlur={e => { const b = JSON.parse(localStorage.getItem('indoo_driver_bank') || '{}'); b.holderName = e.target.value; localStorage.setItem('indoo_driver_bank', JSON.stringify(b)) }} style={{ width: '100%', padding: '12px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: 14, fontWeight: 600, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                </div>
              </div>
              ...
              <div style={{ ...GLASS, padding: 18, marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div>
                    ...
                    <input type="text" placeholder="Full name" defaultValue={savedEmergency.name || ''} onBlur={e => { const em = JSON.parse(localStorage.getItem('indoo_driver_emergency') || '{}'); em.name = e.target.value; localStorage.setItem('indoo_driver_emergency', JSON.stringify(em)) }} style={{ width: '100%', padding: '12px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: 14, fontWeight: 600, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: 6 }}>Phone Number</label>
                    <input type="tel" placeholder="08xxxxxxxxxx" defaultValue={savedEmergency.phone || ''} onBlur={e => { const em = JSON.parse(localStorage.getItem('indoo_driver_emergency') || '{}'); em.phone = e.target.value; localStorage.setItem('indoo_driver_emergency', JSON.stringify(em)) }} style={{ width: '100%', padding: '12px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: 14, fontWeight: 600, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.5)', display: 'block', marginBottom: 6 }}>Relationship</label>
                    <select defaultValue={savedEmergency.relationship || ''} onChange={e => { const em = JSON.parse(localStorage.getItem('indoo_driver_emergency') || '{}'); em.relationship = e.target.value; localStorage.setItem('indoo_driver_emergency', JSON.stringify(em)) }} style={{ width: '100%', padding: '12px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: 14, fontWeight: 600, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box', appearance: 'none' }}>
                      <option value="" disabled style={{ color: '#888' }}>Select relationship...</option>
                      {['Spouse', 'Parent', 'Sibling', 'Other'].map(r => (
                        <option key={r} value={r} style={{ background: '#1a1a1a', color: '#fff' }}>{r}</option>
                      ))}
                    </select>
                  </div>
                </div>
              ...
              {/* ── 6. Vehicle Details ── */}
              <div style={{ ...GLASS, padding: 18, marginBottom: 16 }}>
                <span style={SECTION_TITLE}>🏍️ Vehicle Details</span>
          ...
          <div style={{ width: 300, background: '#0d0d1a', borderLeft: '1px solid rgba(255,255,255,0.1)', display: 'flex', flexDirection: 'column', animation: 'slideLeft 0.3s ease' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px 0' }}>
              ...
              <div style={{ padding: '14px 20px' }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: '#fff', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}><span>🏍️</span> Bike Fare Rates</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 12px' }}>
                  <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>Base Fare</span><span style={{ fontSize: 13, fontWeight: 800, color: '#FACC15' }}>Rp 7.000</span>
                  <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>Per KM</span><span style={{ fontSize: 13, fontWeight: 800, color: '#FACC15' }}>Rp 2.500</span>
                  <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>Min Fare</span><span style={{ fontSize: 13, fontWeight: 800, color: '#FACC15' }}>Rp 10.000</span>
                  <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>Waiting (per min)</span><span style={{ fontSize: 13, fontWeight: 800, color: '#FACC15' }}>Rp 1.000</span>
                </div>
              </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 100px', position: 'relative', zIndex: 1 }}>
            ...
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: 14, padding: 14, borderRadius: 16, background: 'rgba(255,255,255,0.06)', backdropFilter: 'blur(16px)', border: `1px solid ${item.status === 'available' ? item.color + '40' : 'rgba(255,255,255,0.08)'}`, marginBottom: 10 }}>
                {/* Icon/Image */}
                <div style={{ width: 80, height: 80, borderRadius: 16, background: item.status === 'available' ? `${item.color}10` : 'rgba(255,255,255,0.03)', border: `1.5px solid ${item.status === 'available' ? item.color + '30' : 'rgba(255,255,255,0.08)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden', padding: item.img ? 0 : 0 }}>
                  {item.img ? (
