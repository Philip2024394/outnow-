...
export default function SellRentSheet({ open, onClose, onOpenRentalListing, onOpenRentalSell }) {
  if (!open) return null
  ...
  const renderCard = (cat) => (
    <div key={cat.id} style={{ display: 'flex', width: '100%', borderRadius: 20, overflow: 'hidden', height: 110 }}>
      ...
      <button
        ...
        style={{ width: '25%', background: '#6ba32e', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '12px 6px', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}
      >
      ...
      <div style={{ flex: 1, position: 'relative', background: 'rgba(0,0,0,0.75)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', overflow: 'hidden', padding: '10px 10px 8px' }}>
        ...
        <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', fontWeight: 600, zIndex: 1 }}>{cat.desc}</span>
      </div>
