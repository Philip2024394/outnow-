            ...
            return (
              <ScrollReveal key={l.id} delay={Math.min(i * 0.03, 0.3)}>
                <div className="ws-card" onClick={() => onSelectListing?.(l)} style={{ borderRadius: 16, overflow: 'hidden', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.06)', position: 'relative' }}>
                  {isSold && <div style={{ position: 'absolute', inset: 0, zIndex: 5, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 16 }}><span style={{ fontSize: 20, fontWeight: 900, color: l.status === 'sold' ? '#EF4444' : '#60A5FA', padding: '8px 24px', borderRadius: 12, background: 'rgba(0,0,0,0.6)', border: `2px solid ${l.status === 'sold' ? '#EF4444' : '#60A5FA'}` }}>{l.status === 'sold' ? 'SOLD' : 'RENTED'}</span></div>}
